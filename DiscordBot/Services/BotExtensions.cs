...
namespace DiscordBot.Services
{
    public static class BotExtensions
    {
        ...
        public static async Task TryAddDiscordServerIntoDB(this SocketCommandContext context, BotContext dbContext)
        {
            DiscordServer? server = await dbContext.DiscordServers.FindAsync(context.Guild.Id);
            if (server == null)
            {
                await dbContext.DiscordServers.AddAsync(new DiscordServer() { Id = context.Guild.Id });
            }
        }
